...
class ParkingMission:
    def __init__(self):
        rospy.init_node('parking_mission', anonymous=False)
        
        # 파라미터
        
        self.debug_mode = rospy.get_param('~debug_mode', 0.1)
        self.parking_speed = rospy.get_param('~parking_speed', 0.1)
        self.reverse_speed = rospy.get_param('~reverse_speed', 0.1)
        ...
        self.debug_image_pub = rospy.Publisher('/vision/parking_debug', Image, queue_size=10)
        
        rospy.loginfo('주차 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 PARKING인지 확인"""
        if msg.data == 'PARKING':
            if not self.mission_active:
                rospy.loginfo('🅿️ 주차 미션 시작!')
    ...
    def start_parking(self):
        """주차 시작"""
        rospy.loginfo('🅿️ 빈 주차 공간 발견! 주차 시작')
