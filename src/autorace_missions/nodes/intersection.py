...
class IntersectionMission:
    def __init__(self):
        rospy.init_node('intersection_mission', anonymous=False)
        
        # 파라미터
        
        self.debug_mode = rospy.get_param('~debug_mode', 0.1)
        self.turn_speed = rospy.get_param('~turn_speed', 0.1)
        self.turn_duration = rospy.get_param('~turn_duration', 0.1)
        ...
        self.debug_image_pub = rospy.Publisher('/vision/intersection_debug', Image, queue_size=10)
        
        rospy.loginfo('교차로 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 INTERSECTION인지 확인"""
        if msg.data == 'INTERSECTION':
            if not self.mission_active:
                rospy.loginfo('🛣️ 교차로 미션 시작!')
    ...
    def execute_turn(self, direction):
        """회전 실행"""
        rospy.loginfo(f'🛣️ {direction} 방향으로 회전!')
