...
class StopLineMission(Node):
    def __init__(self):
        super().__init__('stop_line_mission')
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/stop_line_debug', 10)
        
        self.get_logger().info('정지선 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 STOP_LINE인지 확인"""
        if msg.data == 'STOP_LINE':
            if not self.mission_active:
                self.get_logger().info('🛑 정지선 미션 시작!')
    ...
    def execute_stop_behavior(self):
        """정지 → 대기 → 출발"""
        self.get_logger().info('🛑 정지선 검출! 정차합니다.')
