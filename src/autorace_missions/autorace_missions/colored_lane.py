...
class ColoredLaneMission(Node):
    def __init__(self):
        super().__init__('colored_lane_mission')
        
        # 파라미터
        self.declare_parameter('debug_mode', True)
        self.declare_parameter('slow_speed', 0.08)      # 빨강
        self.declare_parameter('normal_speed', 0.15)    # 초록
        self.declare_parameter('fast_speed', 0.22)      # 파랑
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/colored_lane_debug', 10)
        
        self.get_logger().info('색깔 차로 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 COLORED_LANE인지 확인"""
        if msg.data == 'COLORED_LANE':
            if not self.mission_active:
                self.get_logger().info('🎨 색깔 차로 미션 시작!')
    ...
    def change_speed(self, color):
        ...
        self.get_logger().info(f'{speed_emoji[color]} {color} 차로 감지! 속도: {speed_map[color]:.2f}m/s')
