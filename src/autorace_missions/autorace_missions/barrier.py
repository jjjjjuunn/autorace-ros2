...
class BarrierMission(Node):
    def __init__(self):
        super().__init__('barrier_mission')
        
        # 파라미터
        self.declare_parameter('debug_mode', True)
        self.declare_parameter('normal_speed', 0.15)
        self.declare_parameter('approach_speed', 0.08)
        self.declare_parameter('barrier_area_threshold', 5000)
        self.declare_parameter('open_angle_threshold', 60)  # 차단기 각도 (도)
        
        self.debug_mode = self.get_parameter('debug_mode').value
        self.normal_speed = self.get_parameter('normal_speed').value
        self.approach_speed = self.get_parameter('approach_speed').value
        self.barrier_area_threshold = self.get_parameter('barrier_area_threshold').value
        self.open_angle_threshold = self.get_parameter('open_angle_threshold').value
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/barrier_debug', 10)
        
        self.get_logger().info('차단기 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 BARRIER인지 확인"""
        if msg.data == 'BARRIER':
            if not self.mission_active:
                self.get_logger().info('🚧 차단기 미션 시작!')
            ...
            if barrier_angle is not None:
                ...
                if barrier_angle > self.open_angle_threshold:
                    if not self.barrier_open:
                        self.get_logger().info('✅ 차단기 열림! 통과합니다.')
                    ...
                    if self.waiting_for_open:
                        ...
                        if self.wait_count % 30 == 0:  # 1초마다 로그
                            self.get_logger().info(f'⏳ 차단기 대기 중... ({self.wait_count // 30}초)')
    ...
    def complete_mission(self):
        """미션 완료"""
        self.get_logger().info('✅ 차단기 통과 완료!')
