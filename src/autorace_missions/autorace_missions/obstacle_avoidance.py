...
class ObstacleAvoidanceMission(Node):
    def __init__(self):
        ...
        self.declare_parameter('obstacle_distance', 0.4)  # 장애물 판단 거리 (m)
        self.declare_parameter('safe_distance', 0.6)      # 안전 거리 (m)
        self.declare_parameter('angular_speed', 0.5)      # 회피 회전 속도
        ...
        self.get_logger().info('라바콘 회피 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 OBSTACLE_AVOIDANCE인지 확인"""
        if msg.data == 'OBSTACLE_AVOIDANCE':
            if not self.mission_active:
                self.get_logger().info('🚧 라바콘 회피 미션 시작!')
    ...
    def start_avoidance(self, left_distance, right_distance):
        ...
        if left_distance > right_distance:
            ...
            self.get_logger().info('⬅️ 좌측으로 회피!')
        else:
            self.avoidance_direction = 'RIGHT'
            self.get_logger().info('➡️ 우측으로 회피!')
    
    def complete_avoidance(self):
        """장애물 회피 완료"""
        self.avoiding = False
        self.obstacles_passed += 1
        self.get_logger().info(f'✅ 라바콘 {self.obstacles_passed}개 회피 완료!')
        
        # 미션 완료 확인
        if self.obstacles_passed >= self.required_obstacles:
            self.get_logger().info('🎉 라바콘 회피 미션 완료!')
            complete_msg = String()
            complete_msg.data = 'OBSTACLE_AVOIDANCE'
            self.mission_complete_pub.publish(complete_msg)
            self.mission_active = False
