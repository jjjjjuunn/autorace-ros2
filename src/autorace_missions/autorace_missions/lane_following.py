...
class LaneFollowingMission(Node):
    def __init__(self):
        super().__init__('lane_following_mission')
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/lane_debug', 10)
        
        self.get_logger().info('차선 추적 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 LANE_FOLLOWING인지 확인"""
        if msg.data == 'LANE_FOLLOWING':
            if not self.mission_active:
                self.get_logger().info('🛣️ 차선 추적 시작!')
