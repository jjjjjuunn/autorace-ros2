...
class TrafficLightMission(Node):
    def __init__(self):
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/traffic_light_debug', 10)
        
        self.get_logger().info('신호등 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 TRAFFIC_LIGHT인지 확인"""
        if msg.data == 'TRAFFIC_LIGHT':
            if not self.mission_active:
                self.get_logger().info('🚦 신호등 미션 시작!')
    ...
    def handle_traffic_light(self, light):
        ...
        if light == 'RED':
            self.get_logger().info('🔴 빨간불! 정지합니다.')
            cmd.linear.x = 0.0
            cmd.angular.z = 0.0
            self.cmd_vel_pub.publish(cmd)
        
        elif light == 'GREEN':
            self.get_logger().info('🟢 초록불! 출발합니다.')
            cmd.linear.x = 0.15
            cmd.angular.z = 0.0
            self.cmd_vel_pub.publish(cmd)
