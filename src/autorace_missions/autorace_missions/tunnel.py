...
class TunnelMission(Node):
    def __init__(self):
        ...
        self.declare_parameter('tunnel_speed', 0.12)
        self.declare_parameter('angular_gain', 0.015)
        self.declare_parameter('brightness_threshold', 80)
        
        self.debug_mode = self.get_parameter('debug_mode').value
        self.tunnel_speed = self.get_parameter('tunnel_speed').value
        ...
        self.debug_image_pub = self.create_publisher(Image, '/vision/tunnel_debug', 10)
        
        self.get_logger().info('터널 미션 노드 준비됨')
    
    def mission_callback(self, msg):
        """현재 미션이 TUNNEL인지 확인"""
        if msg.data == 'TUNNEL':
            if not self.mission_active:
                self.get_logger().info('🚇 터널 미션 시작!')
    ...
    def enter_tunnel(self):
        """터널 진입"""
        self.get_logger().info('🚇 터널 진입!')
        self.in_tunnel = True
    
    def exit_tunnel(self):
        """터널 탈출 - 미션 완료"""
        self.get_logger().info('✅ 터널 통과 완료!')
